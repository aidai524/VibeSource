...
| M2 — 可信提交与人工审核 | 开发者提交、GitHub/Demo 证据、错误状态、审核审计和发布资格形成真实纵向闭环 | M1 | A-101–A-105, A-109, A-111 | In progress — PostgreSQL business repository implemented, external validation pending |
...
- **M2.2b2b2c — PostgreSQL 业务存储（Implemented, locally verified）**：新增候选、审核事件、GitHub/Demo 证据的异步 PostgreSQL 仓储；待审部分唯一索引、短事务、版本锁和数据库级追加保护由 PGlite 验证。SQLite 导入默认只读 dry-run，仅在显式 `--apply`、目标为空和事务内计数一致时写入。真实 Hyperdrive/Neon 未验证。
