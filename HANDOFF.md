...
- Repository state: Git 已初始化，当前分支 `main`；尚无提交
...
- 所有外部证据固定为 `not_checked`；当前代码不会访问 GitHub、Demo 或许可证服务。
...
- 设计 Demo 证据与许可证资格政策；GitHub 的本地失败/过期语义已完成，但生产刷新和保留策略仍待定。
...
- 在 Demo 证据、许可证政策和生产身份完成前，不增加批准或发布入口。
...
- 真实 Demo 可用性、许可证法律/资格判断、生产 GitHub 身份、邮件、支付、分析和生产部署均未接入或验证。
...
实现受控 Demo 可用性证据和许可证资格政策，并选择生产身份与权限方案；评估 GitHub App/服务端身份、条件请求、后台刷新和保留策略。在这些完成前保持 approve/publish 不存在；本地 `node:sqlite` 继续只作为可替换验证适配器。
